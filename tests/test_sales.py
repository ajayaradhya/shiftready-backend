...
@pytest.mark.asyncio
async def test_get_sale_summary_ownership_enforcement(
    async_client, authenticated_user, mock_services
):
...
async def test_publish_sale_logic(async_client, sale_ownership_verified, mock_services):
        ...
        "bundles": [
            {
                ...
                "items": [
                    {
                        "id": "i1",
                        "actual_listing_price": None,
                        "predicted_listing_price": 150.0,
                    },
                    {
                        "id": "i2",
                        "actual_listing_price": 200.0,
                        "predicted_listing_price": 100.0,
                    },
                ],
            }
        ]
    ...
    response = await async_client.post(
        f"/api/v1/sales/{event_id}/publish", json=payload
    )
