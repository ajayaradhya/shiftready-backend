...
class SingleFrameResult(BaseModel):
    ...
    name: str = Field(
        description="Common descriptive name of the item (e.g. 'Armchair', 'Samsung TV')"
    )
    brand: str = Field(
        description="Visible brand/manufacturer, or 'Unknown' if not determinable"
    )
    predicted_original_price: float = Field(
        description="Estimated original retail price in AUD when new"
    )
...
class PricingResult(BaseModel):
    ...
    listing_price: float = Field(
        description="Suggested AUD listing price after urgency discount."
    )
...
class RefinementGrouping(BaseModel):
    ...
    bundle_name: str = Field(
        description="Room name (e.g. 'Living Room', 'Bedroom', 'Kitchen')"
    )
    item_indices: list[int] = Field(
        description="0-based indices into the input items array assigned to this bundle"
    )
